...
log_path = logs_dir / f"onboard_readonly_{time.strftime('%Y%m%d_%H%M%S')}.csv"

print("Onboard read-only flight logger")
print("No movement commands are sent by this script.")
print("This script only reads Pixhawk telemetry, reads camera frames, and writes a CSV log.")
    ...
    while time.time() - start_time < run_time_seconds:
        ...
        if marker_detected:
            ...
            if centered and large_enough:
                suggested_action = "READY_FOR_LAND_LOG_ONLY"
            elif centered:
                suggested_action = "CENTERED_APPROACH_LOG_ONLY"
            else:
                suggested_action = "TRACK_MARKER_LOG_ONLY"

        else:
            suggested_x_command, suggested_y_command, suggested_z_command, suggested_action = (
                get_basic_search_command(elapsed_s)
            )
...
print("Read-only flight logger complete")
